"""
Lambda: Silver Layer Data Quality Validation
────────────────────────────────────────────
Runs automated data quality checks on the Silver layer tables
using Amazon Athena and AWS Wrangler.
...
If one or more checks fail, an alert is sent through Amazon SNS.
The results are returned to AWS Step Functions to determine whether
the pipeline can continue to the Gold layer.

Environment Variables:
    SNS_ALERT_TOPIC_ARN   — SNS topic for data quality failure alerts
    DQ_MIN_ROW_COUNT      — Minimum expected number of rows
    DQ_MAX_NULL_PERCENT   — Maximum allowed null percentage
"""
import os
import json
import logging
...
CRITICAL_COLUMNS = {
    "clean_statics": ["video_id", "title", "channel_title", "views", "region"],
...
def lambda_handler(event, context):
    ...
    """
    database = event.get("database", "youtube_pipeline_silver")
    tables = event.get("tables", ["clean_statics"])
