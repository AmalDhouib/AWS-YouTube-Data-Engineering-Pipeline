"""
Lambda: YouTube API Ingestion to Bronze Layer
──────────────────────────────────────────────
Retrieves trending video statistics and video category reference data
from the YouTube Data API v3 for multiple configured regions.

The Lambda stores the API responses as raw JSON files in the Amazon S3
Bronze layer. Files are organized using Hive-style partitions based on
the region, ingestion date, and ingestion hour.

Two datasets are collected:
  1. Trending video data from the YouTube /videos endpoint
  2. Video category mappings from the /videoCategories endpoint

Technical ingestion metadata is added to each API response before it is
written to Amazon S3. If one or more regional ingestions fail, the Lambda
records the errors and sends a notification through Amazon SNS.

The function is designed to be executed by AWS Step Functions as part of
a scheduled pipeline started by Amazon EventBridge.
...
S3 output structure:
  ...
  youtube/raw-statics-reference-data/
      region=<region>/
      date=<YYYY-MM-DD>/
...
Environment Variables:
    YOUTUBE_API_KEY
        API key used to access the YouTube Data API v3.

    S3_BUCKET_BRONZE
        Name of the Amazon S3 bucket used for the Bronze layer.

    YOUTUBE_REGIONS
        Comma-separated list of region codes to process.
        Default: US,GB,CA,DE,FR,IN,JP,KR,MX,RU

    SNS_ALERT_TOPIC_ARN
        Optional Amazon SNS topic ARN used for failure notifications.
"""
import json
import os
import logging
...
def lambda_handler(event, context):
    ...
    for region in REGIONS:
        ...
        try:
            ...
            # s3://bucket/youtube/raw-statics/region=US/date=2026-03-02/hour=14/data.json
            s3_key = (
                f"youtube/raw-statics/"
                f"region={region}/"
        ...
        try:
            ...
            ref_key = (
                f"youtube/raw-statics-reference-data/"
                f"region={region}/"
                f"date={date_partition}/"
